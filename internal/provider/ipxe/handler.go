...
func (handler *Handler) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if _, err = stateutil.Modify(ctx, handler.state, baremetal.NewMachineStatus(uuid), func(status *baremetal.MachineStatus) error {
...
func (handler *Handler) makeBootDecision(ctx context.Context, arch, uuid string, logger *zap.Logger) (bootDecision, error) {
	...
	ress, err := handler.getResources(ctx, uuid)
	if err != nil {
	...
	if !ress.exists { // we see the machine for the first time, boot into agent mode
		requiredBootMode = specs.BootMode_BOOT_MODE_AGENT_PXE
	} else {
		mode, modeErr := boot.DetermineRequiredMode(ress.infraMachine, ress.status, ress.installStatus, logger)
		if modeErr != nil {
	...
	case specs.BootMode_BOOT_MODE_AGENT_PXE:
		body, statusCode, agentErr := handler.bootIntoAgentMode(ctx, arch)
		if agentErr != nil {
	...
	case specs.BootMode_BOOT_MODE_TALOS_PXE:
		...
		defaultExtraKernelArgs := slices.Concat(handler.defaultKernelArgs, consoleKernelArgs)
		talosVersion := ress.infraMachine.TypedSpec().Value.ClusterTalosVersion
		extensions := ress.infraMachine.TypedSpec().Value.Extensions

		var ipxeURL string

		ipxeURL, err = handler.imageFactoryClient.SchematicIPXEURL(ctx, false, talosVersion, arch, extensions, defaultExtraKernelArgs)
		if err != nil {
...
func (handler *Handler) getResources(ctx context.Context, id string) (resources, error) {
	...
	status, err := safe.StateGetByID[*baremetal.MachineStatus](ctx, handler.state, id)
	if err != nil {
...
func (handler *Handler) bootIntoAgentMode(ctx context.Context, arch string) (string, int, error) {
	agentExtraKernelArgs := slices.Concat(handler.agentKernelArgs, handler.consoleKernelArgs(arch))
