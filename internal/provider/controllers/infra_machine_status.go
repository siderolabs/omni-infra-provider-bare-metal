...
import (
	...
	"github.com/siderolabs/omni-infra-provider-bare-metal/internal/provider/stateutil"
)
...
// AgentController is the interface for interacting with the Talos agent over the reverse GRPC tunnel.
...
func NewInfraMachineController(agentController AgentController, apiPowerManager APIPowerManager, state state.State, requeueInterval time.Duration) *InfraMachineController {
...
func (h *infraMachineControllerHelper) transform(ctx context.Context, reader controller.Reader, logger *zap.Logger,
	...
	if err != nil {
		if state.IsNotFoundError(err) {
			return controller.NewRequeueErrorf(h.requeueInterval, "baremetal.MachineStatus not available yet, requeue")
		}

		return err
	}

	powerState := status.TypedSpec().Value.PowerState
	bootMode := status.TypedSpec().Value.BootMode
	lastWipeID := status.TypedSpec().Value.LastWipeId

	logger = logger.With(
		zap.Stringer("power_state", powerState),
		zap.Stringer("boot_mode", bootMode),
	...
	installStatus, err := safe.ReaderGetByID[*infra.MachineState](ctx, reader, infraMachine.Metadata().ID())
	if err != nil && !state.IsNotFoundError(err) {
		return err
	}

	mode, err := boot.DetermineRequiredMode(infraMachine, status, installStatus, logger)
	if err != nil {
		return err
	}
	...
	// Switching from PXE booted Talos to booting from disk does not require a reboot by the provider,
	// as Omni itself will do the switch.
...
func (h *infraMachineControllerHelper) finalizerRemoval(ctx context.Context, reader controller.Reader, logger *zap.Logger, infraMachine *infra.Machine) (retErr error) {
	if err := h.finalize(ctx, reader, infraMachine, logger); err != nil {
		if xerrors.TypeIs[*controller.RequeueError](err) { // if the returned error is a requeue, respect it
			return err
		}

		logger.Warn("failed to finalize machine, but remove finalizer anyway", zap.Error(err))
	}

	return nil
}

func (h *infraMachineControllerHelper) finalize(ctx context.Context, reader controller.Reader, infraMachine *infra.Machine, logger *zap.Logger) error {
	status, err := safe.ReaderGetByID[*baremetal.MachineStatus](ctx, reader, infraMachine.Metadata().ID())
	if err != nil {
		if state.IsNotFoundError(err) {
			return nil
	...
	// the machine needs to be wiped, wipe it
	if err = h.wipe(ctx, infraMachine.Metadata().ID(), "", logger); err != nil {
		return fmt.Errorf("failed to wipe machine: %w", err)
	}
...
func (h *infraMachineControllerHelper) wipe(ctx context.Context, id resource.ID, pendingWipeID string, logger *zap.Logger) error {
	if err := h.agentController.WipeDisks(ctx, id); err != nil {
	...
	if _, err := stateutil.Modify(ctx, h.state, baremetal.NewMachineStatus(id), func(res *baremetal.MachineStatus) error {
...
func (h *infraMachineControllerHelper) ensurePowerManagement(ctx context.Context, status *baremetal.MachineStatus, logger *zap.Logger) error {
	...
	powerManagement, err := h.agentController.GetPowerManagement(ctx, id)
	if err != nil {
	...
	_, err = stateutil.Modify(ctx, h.state, baremetal.NewMachineStatus(id), func(status *baremetal.MachineStatus) error {
...
func (h *infraMachineControllerHelper) ensurePowerManagementOnAgent(ctx context.Context, id resource.ID, powerManagement *agentpb.GetPowerManagementResponse) (ipmiPassword string, err error) {
	...
	if err = h.agentController.SetPowerManagement(ctx, id, &agentpb.SetPowerManagementRequest{
		Api:  api,
		Ipmi: ipmi,
	}); err != nil {
		if grpcstatus.Code(err) == codes.Unavailable {
