...
func (m *Poller) poll(ctx context.Context) {
	...
	for status := range statusList.All() {
		...
		agentConnected, connectedErr := m.agentConnected(ctx, connectedMachines, status.Metadata().ID())
		if connectedErr != nil && !errors.Is(connectedErr, context.Canceled) {
			m.logger.Error("failed to check connection", zap.String("machine_id", status.Metadata().ID()), zap.Error(connectedErr))
		...
		if _, saveErr := stateutil.Modify(ctx, m.state, baremetal.NewMachineStatus(status.Metadata().ID()), func(status *baremetal.MachineStatus) error {
