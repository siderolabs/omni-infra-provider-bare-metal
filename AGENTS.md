...
The heart of the lifecycle is `machine.RequiredBootMode` (`internal/provider/machine/machine.go`), which decides whether a machine should PXE into agent mode, PXE into Talos, or boot from disk.
...
5. On allocation to a cluster, the provider powers the machine on with a one-time PXE boot into Talos (non-agent), serving a Talos schematic built from the cluster's Talos version, extensions, and kernel args.
   Talos installs to disk and joins the cluster.
6. In steady state the provider tells the machine to boot from disk on subsequent PXE hits.
7. On deprovision, the provider PXE-boots the machine back into agent mode, wipes disks again, and powers it off, returning it to the pool.
