...
fetch-source-assets:  ## Fetches the source assets into the source tree, for building natively outside docker.
	rm -rf -- 'internal/provider/ipxe/data/amd64/ipxe.efi' 'internal/provider/ipxe/data/amd64/snp.efi' 'internal/provider/ipxe/data/amd64/kpxe/undionly.kpxe.bin' 'internal/provider/ipxe/data/amd64/kpxe/undionly.kpxe.zinfo' 'internal/provider/ipxe/data/arm64/ipxe.efi' 'internal/provider/ipxe/data/arm64/snp.efi'
...
.PHONY: sign-image-provider
sign-image-provider:  ## Signs the image for omni-infra-provider-bare-metal. Requires interactive Google authentication.
	@test -n "$$GITHUB_TOKEN" || { \
	...
	@TMP=$$(mktemp -d) && trap 'rm -rf "$$TMP"' EXIT && \
	  ...
	  docker run --rm -p 127.0.0.1:8585:8585 \
	    ...
	    $(REGISTRY)/$(USERNAME)/omni-infra-provider-bare-metal:$(IMAGE_TAG)
